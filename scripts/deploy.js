...
async function main() {
  ...
  const artifactsPath = path.join(
    __dirname,
    "../contracts/artifacts/contracts/contracts",
  );

  const crowdfundingArtifact = JSON.parse(
    fs.readFileSync(
      path.join(artifactsPath, "Crowdfunding.sol/Crowdfunding.json"),
    ),
  );
  fs.writeFileSync(
  ...
  const rewardTokenArtifact = JSON.parse(
    fs.readFileSync(
      path.join(artifactsPath, "RewardToken.sol/RewardToken.json"),
    ),
  );
  fs.writeFileSync(
